...
Fluid::Fluid(int container_size, float scaling, float diffusion,
...
void Fluid::advect(FieldType b, Field<float> &d, Field<float> &d0,
                   Field<float> &velocX, Field<float> &velocY,
                   Field<float> &velocZ) {
    float dtx = dt * (N - 2);
    float dty = dt * (N - 2);
    float dtz = dt * (N - 2);
...
void Fluid::diffuse(FieldType b, Field<float> &x, Field<float> &x0,
                    float diff) {
    float a = dt * diff * pow(container_size - 2, 3);
    lin_solve(b, x, x0, a, 1 + 6 * a);
}

void Fluid::lin_solve(FieldType b, Field<float> &f, Field<float> &f0, float a,
                      float c) {
    float cRecip = 1.0f / c;
...
#pragma omp parallel for collapse(2)
            for (int y = 1; y < N - 1; y++) {
                for (int x = 1; x < N - 1; x++) {
                    f[IX(x, y, z)] =
                        (f0[IX(x, y, z)] +
                         a * (f[IX(x + 1, y, z)] + f[IX(x - 1, y, z)] +
                              f[IX(x, y + 1, z)] + f[IX(x, y - 1, z)] +
                              f[IX(x, y, z + 1)] + f[IX(x, y, z - 1)])) *
                        cRecip;
                }
            }
        }
...
void Fluid::project(Field<float> &velocX, Field<float> &velocY,
                    Field<float> &velocZ, Field<float> &p, Field<float> &div) {
        ...
        for (int y = 1; y < N - 1; y++) {
            for (int x = 1; x < N - 1; x++) {
                if (state[IX(x, y, z)] == CellType::SOLID) {
                    ...
                    velocX[IX(x, y, z)] -=
                        0.5f * fraction *
                        (p[IX(x + 1, y, z)] - p[IX(x - 1, y, z)]) * N;
                    velocY[IX(x, y, z)] -=
                        0.5f * fraction *
                        (p[IX(x, y + 1, z)] - p[IX(x, y - 1, z)]) * N;
                    velocZ[IX(x, y, z)] -=
                        0.5f * fraction *
                        (p[IX(x, y, z + 1)] - p[IX(x, y, z - 1)]) * N;
                } else { // FLUID cells
                    velocX[IX(x, y, z)] -=
                        0.5f * (p[IX(x + 1, y, z)] - p[IX(x - 1, y, z)]) * N;
                    velocY[IX(x, y, z)] -=
                        0.5f * (p[IX(x, y + 1, z)] - p[IX(x, y - 1, z)]) * N;
                    velocZ[IX(x, y, z)] -=
                        0.5f * (p[IX(x, y, z + 1)] - p[IX(x, y, z - 1)]) * N;
                }
            }
        }
    }
...
#pragma omp parallel for collapse(2)
    ...
    f[IX(0, N - 1, 0)] =
        0.33f * (f[IX(1, N - 1, 0)] + f[IX(0, N - 2, 0)] + f[IX(0, N - 1, 1)]);
    f[IX(0, 0, N - 1)] =
        0.33f * (f[IX(1, 0, N - 1)] + f[IX(0, 1, N - 1)] + f[IX(0, 0, N - 2)]);
    f[IX(0, N - 1, N - 1)] =
        0.33f * (f[IX(1, N - 1, N - 1)] + f[IX(0, N - 2, N - 1)] +
                 f[IX(0, N - 1, N - 2)]);
    f[IX(N - 1, 0, 0)] =
        0.33f * (f[IX(N - 2, 0, 0)] + f[IX(N - 1, 1, 0)] + f[IX(N - 1, 0, 1)]);
    f[IX(N - 1, N - 1, 0)] =
        0.33f * (f[IX(N - 2, N - 1, 0)] + f[IX(N - 1, N - 2, 0)] +
                 f[IX(N - 1, N - 1, 1)]);
    f[IX(N - 1, 0, N - 1)] =
        0.33f * (f[IX(N - 2, 0, N - 1)] + f[IX(N - 1, 1, N - 1)] +
                 f[IX(N - 1, 0, N - 2)]);
    f[IX(N - 1, N - 1, N - 1)] =
        0.33f * (f[IX(N - 2, N - 1, N - 1)] + f[IX(N - 1, N - 2, N - 1)] +
                 f[IX(N - 1, N - 1, N - 2)]);
}
    ...
    for (int z = 0; z < N; z++) {
        for (int y = 0; y < N; y++) {
            for (int x = 0; x < N; x++) {
                ...
                auto cell_geometry = std::make_shared<fcl::Boxf>(
                ...
                if (result.isCollision()) {
                    // Simplify: Assume SOLID if collision exists (for
                    // debugging)
                    ...
                    fcl::AABBf cell_aabb(cell_position, cell_position +
                    cell_size);
                    ...
                    contact.pos.transpose() << std::endl; if
                    (!cell_aabb.contain(contact.pos)) { fully_inside = false;
                            break;
                        }
                    }
...
float Fluid::get_volume(v3 cell_position) {
    ...
    for (auto &obstacle : obstacles) {
        fcl::AABBf obstacle_aabb(obstacle->geom->aabb_local.min_ +
                                     (fcl::Vector3f)obstacle->position,
                                 obstacle->geom->aabb_local.max_ +
                                     (fcl::Vector3f)obstacle->position);
        ...
        fcl::Vector3f intersection_size =
            intersection_aabb.max_ - intersection_aabb.min_;
        if (intersection_size[0] <= 0 || intersection_size[1] <= 0 ||
            intersection_size[2] <= 0) {
            return 0.0f;
        }
...
// float Fluid::get_volume(v3 cell_position) { return
// volume[IXv(cell_position)]; }
